...
int nget(char *src, char *dst)
{
	...
	if (username)
	{
		...
		fujiF5_write(DEVICEID_FN_NETWORK, CMD_USERNAME, FUJI_FIELD_NONE, 0, 0, &url, sizeof(url));
	}
	...
	if (password)
	{
		...
		fujiF5_write(DEVICEID_FN_NETWORK, CMD_PASSWORD, FUJI_FIELD_NONE, 0, 0, &url, sizeof(url));
	}
	...
	fujiF5_write(DEVICEID_FN_NETWORK, CMD_OPEN, FUJI_FIELD_A1_A2, 0x04, 0, url, sizeof(url));

 	delay(10);

	/* Perform initial status command */
	fujiF5_read(DEVICEID_FN_NETWORK, CMD_STATUS, FUJI_FIELD_NONE, 0, 0, &status, sizeof(status));
	...
	while (1)
	{
		...
		/* Do read */
		reply = fujiF5_read(DEVICEID_FN_NETWORK, CMD_READ, FUJI_FIELD_B12, bw, 0, &buf, bw);
		...
		/* Do next status */
		fujiF5_read(DEVICEID_FN_NETWORK, CMD_STATUS, FUJI_FIELD_B12, 0, 0, &status, sizeof(status));
	}

	/* Perform CLOSE command */
	fujiF5_none(DEVICEID_FN_NETWORK, CMD_CLOSE, FUJI_FIELD_NONE, 0, 0, NULL, 0);
